...
struct liste* creerListeDeContact(struct Contact* contact)
{
    struct liste* nouvelle = (struct liste*)malloc(sizeof(struct liste));

    nouvelle->var = (struct Contact*)contact;
    nouvelle->suivant = NULL;

    return nouvelle;
}

struct liste* creerListeEntier(int* entier)
{
    struct liste* nouvelle = (struct liste*)malloc(sizeof(struct liste));

    nouvelle->var = (int*)entier;
    nouvelle->suivant = NULL;
